...
/// <c>database update</c>). The connection string is read from the <c>HTB_CONNECTION_STRING</c>
/// environment variable, falling back to a local default. Migrations target the read-only
/// <see cref="StrategyReadonlyDbContext"/> (the migration target that owns the snapshot) and live in
/// this assembly, separate from the context in HTB.Shared, so the assembly name is pinned explicitly.
/// Mirrors <c>MarketDataReadonlyDbContextFactory</c>.
/// </summary>
public sealed class StrategyReadonlyDbContextFactory
    : IDesignTimeDbContextFactory<StrategyReadonlyDbContext>
{
    ...
    // Used at build time when no env var is present. EF only needs a syntactically valid string to
    // construct the model; it never opens a connection. The real connection is supplied at runtime
    // via HTB_CONNECTION_STRING or the bundle's `--connection` flag.
